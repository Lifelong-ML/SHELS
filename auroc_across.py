...
class_list = [1]
save_path = "multiplw/MNIST/model1"
if len(sys.argv) > 1:
...
for i in range(1,4):
    ...
    #
    # ood_class_list = curr_list[23:43]
    # ind_class_list = curr_list[0:23]
    #
    # classes_ind = np.array(classes)[ind_class_list]
    # ood_classes = np.array(classes)[ood_class_list]
    ood_classes = classes
    model = torch.load(save_path+'/seed{}/model_{}.pt'.format(i,0), map_location='cpu')
    ...
    activations_test = dict(np.load(save_path+'/seed{}/activations/act_full_test_{}.npz'.format(i,0),allow_pickle = True))
    ...
    for j in range(0, 1):
        ...
        activations_ood = dict(np.load(save_path+'/seed{}/activations/act_full_ood_{}_{}.npz'.format(i,ood_classes[j],0), allow_pickle = True))['act_ood']
